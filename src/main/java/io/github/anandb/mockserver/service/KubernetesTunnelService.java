...
public class KubernetesTunnelService {
    private static final Logger log = LoggerFactory.getLogger(KubernetesTunnelService.class);
    ...
    public String discoverPod(String namespace, String podPrefix) throws IOException, InterruptedException {
        ProcessBuilder pb = new ProcessBuilder("kubectl", "get", "pods", "-n", namespace, "--no-headers", "-o", "custom-columns=:metadata.name");
        pb.redirectErrorStream(true);
        Process process = pb.start();
        StringBuilder output = new StringBuilder();
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(process.getInputStream()))) {
    ...
    public Process startTunnel(TunnelConfig config, int hostPort) throws IOException, InterruptedException {
        ...
        log.info("Starting kubectl port-forward for pod: {} in namespace: {} on local port: {} to pod port: {}", podName, config.getNamespace(), hostPort, config.getPodPort());
        ProcessBuilder pb = new ProcessBuilder("kubectl", "port-forward", "--address", "127.0.0.1", "pod/" + podName, hostPort + ":" + config.getPodPort(), "-n", config.getNamespace());
        pb.redirectErrorStream(true);
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (canConnect(hostPort)) {
                log.info("Tunnel ready on port {} after {}ms", hostPort, config.getTunnelReadyTimeoutMs() - (deadline - System.currentTimeMillis()));
                return process;
            }
        ...
        // Timeout — clean up
        process.destroyForcibly();
        throw new IOException("Tunnel failed to become ready on port " + hostPort + " within " + config.getTunnelReadyTimeoutMs() + "ms");
    }
