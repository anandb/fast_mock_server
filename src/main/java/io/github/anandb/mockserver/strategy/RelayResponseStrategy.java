...
public class RelayResponseStrategy implements ResponseStrategy {
    private static final Logger log = LoggerFactory.getLogger(RelayResponseStrategy.class);
    ...
    public HttpResponse handle(HttpRequest httpRequest, EnhancedExpectation config, Map<String, Object> context) {
        try {
            ...
            if (httpRequest.getQueryStringParameters() != null && !httpRequest.getQueryStringParameters().isEmpty()) {
                String queryString = httpRequest.getQueryStringParameters().getEntries().stream().map(param -> param.getName().getValue() + "=" + String.join(",", param.getValues().stream().map(val -> val.getValue()).toList())).collect(java.util.stream.Collectors.joining("&"));
                relayPath = path + "?" + queryString;
            }
            // Early log: helps when later request parsing fails or logs get delayed.
            log.info("[RelayResponseStrategy] Incoming request (early) -> method={} path={} relayPath={}", method, path, relayPath);
            String requestHeaders = "";
            try {
                requestHeaders = RequestUtils.headersToMap(httpRequest).entrySet().stream().map(entry -> entry.getKey() + "=" + String.join(",", entry.getValue())).collect(Collectors.joining("; "));
            ...
            log.info("[RelayResponseStrategy] Incoming request -> method={} path={} relayPath={} headers=[{}] body=[{}]", method, path, relayPath, requestHeaders, bodyString);
            ...
            if (matchingRelay.isEmpty()) {
                ...
                return HttpResponse.response().withStatusCode(404).withBody(new ErrorCode("NO_MATCHING_RELAY", "No matching relay configuration found for: " + path).toString());
            }
            RelayResponse relayResponse = relayService.relayRequest(matchingRelay.get(), method, relayPath, headers, body);
            String relayResponseHeaders = "";
            if (relayResponse.headers() != null && !relayResponse.headers().isEmpty()) {
                relayResponseHeaders = relayResponse.headers().entrySet().stream().filter(entry -> entry.getKey() != null && !entry.getKey().startsWith(":")).map(entry -> entry.getKey() + "=" + String.join(",", entry.getValue())).collect(Collectors.joining("; "));
            }
            ...
            log.info("[RelayResponseStrategy] Relayed response <- statusCode={} headers=[{}] body=[{}] (relayPath={})", relayResponse.statusCode(), relayResponseHeaders, relayResponseBody, relayPath);
            HttpResponse response = HttpResponse.response().withStatusCode(relayResponse.statusCode());
            if (relayResponse.headers() != null) {
                for (Map.Entry<String, List<String>> header : relayResponse.headers().entrySet()) {
                    ...
                    if (header.getValue() != null && !header.getValue().isEmpty()) {
                        response = response.withHeader(header.getKey(), header.getValue().toArray(String[]::new));
                    }
                }
            }
            if (relayResponse.body() != null && relayResponse.body().length > 0) {
            ...
            if (response != null) {
                log.info("[RelayResponseStrategy] Final response -> statusCode={} (contentLength={})", relayResponse.statusCode(), relayResponse.body() == null ? 0 : relayResponse.body().length);
            }
            return response;
        } catch (Exception e) {
            log.error("Error relaying request", e);
            return HttpResponse.response().withStatusCode(502).withBody(new ErrorCode("RELAY_ERROR", "Error relaying request to remote server: " + e.getMessage()).toString());
        }
    }
    ...
    private boolean isLoggableTextContentType(String contentType) {
        ...
        return lower.contains("json") || lower.contains("xml") || lower.contains("text") || lower.contains("html") || lower.contains("javascript");
    }

    public RelayResponseStrategy(final RelayService relayService) {
